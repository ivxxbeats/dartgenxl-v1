...
(function() {
    ...
    function weightedPick(items, weights, rng) {
        ...
        let roll = rng() * total;
        for (let i = 0; i < items.length; i++) { if (roll < weights[i]) return items[i]; roll -= weights[i]; }
    ...
    function rollRarityClass(rng) { const r = rng(); if (r < 0.60) return RARITY_CLASSES.COMMON; if (r < 0.85) return RARITY_CLASSES.UNCOMMON; if (r < 0.95) return RARITY_CLASSES.RARE; if (r < 0.99) return RARITY_CLASSES.MYTHIC; return RARITY_CLASSES.GRAIL; }
    function rollArchetype(rarityClass, rng) { const weights = { [RARITY_CLASSES.COMMON]: [0.20,0.18,0.15,0.17,0.18,0.12], [RARITY_CLASSES.UNCOMMON]: [0.17,0.17,0.17,0.17,0.18,0.14], [RARITY_CLASSES.RARE]: [0.14,0.16,0.20,0.16,0.18,0.16], [RARITY_CLASSES.MYTHIC]: [0.10,0.14,0.22,0.14,0.20,0.20], [RARITY_CLASSES.GRAIL]: [0.12,0.14,0.18,0.12,0.28,0.16] }; return weightedPick(ARCHETYPES, weights[rarityClass] || weights[RARITY_CLASSES.COMMON], rng); }
    function rollAnchorForm(archetype, rng) { const weights = { "Signal": [0.10,0.28,0.08,0.30,0.18,0.06], "Drift": [0.18,0.22,0.06,0.08,0.32,0.14], "Rift": [0.06,0.08,0.46,0.12,0.10,0.18], "Core": [0.32,0.26,0.06,0.16,0.10,0.10], "Prism": [0.12,0.30,0.10,0.20,0.16,0.12], "Void": [0.28,0.16,0.18,0.14,0.08,0.16] }; return weightedPick(ANCHOR_FORMS, weights[archetype] || weights.Signal, rng); }
    function rollEngineType(rng, rarityClass) { if (rarityClass === RARITY_CLASSES.GRAIL) return weightedPick(ENGINE_TYPES, [0.10,0.20,0.70], rng); return weightedPick(ENGINE_TYPES, [0.78,0.19,0.03], rng); }
    function rollPrimaryDriver(rng) { return weightedPick(PRIMARY_DRIVERS, [0.35,0.25,0.25,0.15], rng); }
    function rollStructureType(rng) { return STRUCTURE_TYPES[Math.floor(rng() * STRUCTURE_TYPES.length)]; }
    function rollColorMood(rng) { return COLOR_MOODS[Math.floor(rng() * COLOR_MOODS.length)]; }
    function rollSpatialBehavior(engineType, rng, extendedPool = true) { 
        const pool = extendedPool ? SPATIAL_BEHAVIORS_EXTENDED : SPATIAL_BEHAVIORS;
        const allowed = {
    ...
    function rollFailureMode(rng, engineType, rarityClass) { if (rarityClass === RARITY_CLASSES.GRAIL) { if (engineType === "Rupture") return weightedPick(FAILURE_MODES, [0.05,0.10,0.15,0.70], rng); if (engineType === "Echo") return weightedPick(FAILURE_MODES, [0.10,0.45,0.30,0.15], rng); return weightedPick(FAILURE_MODES, [0.30,0.35,0.25,0.10], rng); } if (engineType === "Canonical") return weightedPick(FAILURE_MODES, [0.70,0.20,0.08,0.02], rng); if (engineType === "Echo") return weightedPick(FAILURE_MODES, [0.30,0.45,0.20,0.05], rng); return weightedPick(FAILURE_MODES, [0.15,0.20,0.15,0.50], rng); }
    function rollAnomalyClass(rng) { return weightedPick(ANOMALY_CLASSES, [0.25,0.25,0.25,0.25], rng); }
    ...
    function generateTraits(seed, tokenId) {
        const streams = {}; for (let i = 1; i <= 12; i++) streams[i] = makeSeededRand(splitSeed(seed, i));
        const tokenNum = parseInt(tokenId, 10) || 0;
        const steps = (tokenNum * 997) % 1000;
        for (let i = 0; i < steps; i++) for (let s = 1; s <= 12; s++) if (streams[s]) streams[s]();
        const rng = streams[1];
        ...
        // 🆕 MICRO-VARIANTS (hidden, but affect visual output)
        ...
        // 🆕 MICRO EVENT (rare, deterministic)
    ...
    function generateBaseTraits(seed, tokenId) {
        const streams = {}; for (let i = 1; i <= 12; i++) streams[i] = makeSeededRand(splitSeed(seed, i + 100));
        const tokenNum = parseInt(tokenId, 10) || 0;
        const steps = (tokenNum * 997) % 1000;
        for (let i = 0; i < steps; i++) for (let s = 1; s <= 12; s++) if (streams[s]) streams[s]();
        const rng = streams[1];
    ...
    function getCanonicalIntensity(seed) { const rng = makeSeededRand(splitSeed(seed, 200)); return 0.32 + rng() * 0.58; }
    function getCanonicalTime(tokenId, seed, intensity) { const tokenNum = parseInt(tokenId, 10) || 0; return ((tokenNum * 0.0123456789) + (seed * 0.0000001) + (intensity * 0.1)) % 1.0; }
    
    // ============================================================
    // GEOMETRY & ENGINE HELPERS (with micro-variant support)
    // ============================================================
    
    function applyArchetypeGeometry(archetype, x, y) { let rx = x, ry = y; switch(archetype) { case "Signal": rx *= 0.8; ry *= 0.8; break; case "Drift": rx += Math.sin(ry * 2) * 0.4; break; case "Rift": rx *= 1.6; break; case "Core": ry *= 0.5; break; case "Prism": rx = Math.abs(rx); ry = Math.abs(ry); break; case "Void": rx *= -1; ry *= -1; break; } return { x: rx, y: ry }; }
    ...
    function reinforceArchetype(archetype, rx, ry, pressure) {
        ...
        switch(archetype) {
            ...
            case "Core": const centerPull = Math.exp(-(x * x + y * y) * (1.8 - pressure * 0.5)); x = x * (0.96 + centerPull * 0.04); y = y * (0.96 + centerPull * 0.04); break;
            case "Prism": const sharpness = 0.92 - pressure * 0.04; x = Math.sign(x) * Math.pow(Math.abs(x), sharpness); y = Math.sign(y) * Math.pow(Math.abs(y), sharpness); break;
            case "Void": const voidFalloff = 1.0 - Math.exp(-(x * x + y * y) * (1.2 + pressure * 0.5)); x = x * (0.94 + voidFalloff * 0.06); y = y * (0.94 + voidFalloff * 0.06); break;
        }
        return { x, y };
    }
    ...
    function reinforceAnchor(t, anchorForm, rx, ry, pressure) {
        ...
        if (anchorForm === "Gate") { const gate = Math.exp(-Math.pow(rx * 0.9, 2) * 6.0) * amp; result = t * (0.88 - pressure * 0.03) + gate * (0.12 + pressure * 0.04); }
        else if (anchorForm === "Nexus") { const nexus = Math.exp(-(rx * rx + ry * ry) * 2.6) * amp * 1.2; result = t * (0.84 - pressure * 0.04) + nexus * (0.16 + pressure * 0.06); }
        else if (anchorForm === "Faultline") { const fault = Math.abs(Math.sin(rx * 10.0 - ry * 6.0)) * amp; result = t * (0.86 - pressure * 0.03) + fault * (0.14 + pressure * 0.05); }
        else if (anchorForm === "Bloom") { const r = Math.sqrt(rx * rx + ry * ry); const bloom = (Math.sin(r * 14.0) * 0.5 + 0.5) * amp; result = t * (0.88 - pressure * 0.03) + bloom * (0.12 + pressure * 0.04); }
        else if (anchorForm === "PrismHeart") { const prism = (Math.abs(rx) + Math.abs(ry)) * amp * 0.8; result = t * (0.90 - pressure * 0.02) + prism * (0.10 + pressure * 0.03); }
        else if (anchorForm === "Aether") { const aether = (Math.sin((rx + ry) * 5.0) * 0.5 + 0.5) * amp * 0.7; result = t * (0.92 - pressure * 0.02) + aether * (0.08 + pressure * 0.02); }
        return Math.max(0.03, Math.min(0.97, result));
    }
    
    function novaFractalCalc(x0, y0, maxIter) {
        let x = x0, y = y0; let iter = 0;
        for (iter = 0; iter < maxIter; iter++) { const x2 = x * x, y2 = y * y; if (x2 + y2 > 4.0) break; const xt = x2 - y2 + x0; y = 2.0 * x * y + y0; x = xt; }
        let smooth; if (iter < maxIter) { const mag2 = x * x + y * y; const logZn = Math.log(Math.max(mag2, 1e-10)) * 0.5; const nu = Math.log(logZn / LOG2) / LOG2; smooth = (iter + 1 - nu) / maxIter; } else smooth = 1.0;
        return Math.max(0.02, Math.min(0.98, smooth));
    }
    
    function getDepthFractalValue(x, y, maxIter, layers, iterMult) { let depth = 0; for (let i = 0; i < layers; i++) { const scale = 1 + i * 0.15; depth += novaFractalCalc(x * scale, y * scale, Math.floor(maxIter * iterMult * 0.7)); } return depth / layers; }
    
    function getPatternValue(x, y, time, engineName, pressure, primaryDriver, rx, ry, liveTime, microWarp, phaseBias, motionVariant) {
        const r = Math.sqrt(x * x + y * y); const a = Math.atan2(y, x);
        let val = 0;
        // Apply micro-warp and phase bias to pattern
        ...
        // Motion variant affects pattern mixing
    ...
    function applyCompositionTransform(ux, uy, composition, zoom, offsetX, offsetY, symmetryBreak) {
        let x = ux / zoom + offsetX; let y = uy / zoom + offsetY;
        
        // Apply symmetry break
        x += symmetryBreak * 0.2;
    ...
    function getRichColor(t, colorMood, time, primaryDriver, pressure, paletteVariant) {
        ...
        // Apply palette variant (subtle hue shifts)
    ...
    function signatureColor(t, time, pressure) { const base = t * (30 + pressure * 15) + time; return { r: Math.sin(base) * 0.5 + 0.5, g: Math.sin(base + 2.094) * 0.5 + 0.5, b: Math.sin(base + 4.188) * 0.5 + 0.5 }; }
    
    function engineFrequencyShape(t, engineName, freqMultiplier, pressure, engineVariant) {
        let out;
        // Engine variant affects frequency behavior
        const variantFactor = engineVariant === "Breathing" ? 0.9 : engineVariant === "Pulsing" ? 1.1 : engineVariant === "Harmonic" ? 1.2 : 1.0;
    ...
    function engineContrastShape(t, engineName, pressure, textureVariant) {
        const k = 6.0; let s = 1.0 / (1.0 + Math.exp(-k * (t - 0.5))); 
        s = s * 0.9 + Math.abs(Math.sin(t * Math.PI)) * 0.1;
        
        // Texture variant affects contrast curve
    ...
    function engineColorDiscipline(r, g, b, engineName, t, time, pressure) {
        if (engineName === "Canonical") { const intensity = 0.96 - pressure * 0.04; return { r: r * intensity, g: g * intensity, b: b * intensity }; }
        if (engineName === "Echo") { const bleed = 0.04 + pressure * 0.06; return { r: r * (0.96 - pressure * 0.04) + (Math.sin(time + t * 4) * 0.5 + 0.5) * bleed, g: g * (0.96 - pressure * 0.04) + (Math.sin(time + 2.094 + t * 4) * 0.5 + 0.5) * bleed, b: b * (0.96 - pressure * 0.04) + (Math.sin(time + 4.188 + t * 4) * 0.5 + 0.5) * bleed }; }
        return { r: Math.min(1, r * (1.06 + pressure * 0.08)), g: g * (0.88 - pressure * 0.06), b: Math.min(1, b * (1.03 + pressure * 0.05)) };
    }
    ...
    function applyEngineLiveBehavior(t, engineType, liveIntensity, liveTime, pressure, traits, rx, ry, echoMemoryBuffer) {
    ...
    function computePixel(options) {
        ...
        const { zoom, offsetX, offsetY, baseMaxIter, layers, iterMult, microWarp, grainScale, phaseBias, symmetryBreak } = baseTraits;
        ...
        if (spatialBehavior === "Vortex") { let vr = Math.sqrt(rx * rx + ry * ry); let va = Math.atan2(ry, rx); va += vr * 3.0; rx = Math.cos(va) * vr * 0.8; ry = Math.sin(va) * vr * 0.8; }
        
        if (isRupture) { rx += Math.sin(ry * 6 + time * 4) * (0.15 + pressure * 0.1); ry += Math.cos(rx * 6 - time * 4) * (0.15 + pressure * 0.1); }
        ...
        let t;
        if (isRupture) { t = Math.abs(fractalVal - patternVal) + Math.sin(rx * ry * 2.5) * 0.2; }
        else if (isEcho) { t = fractalVal * 0.35 + patternVal * 0.65; t = t * 0.8 + Math.sin(t * Math.PI * 2) * 0.2; t = t * 0.8 + Math.sin(t * Math.PI * 2 + Math.sin(t * 6)) * 0.2; }
        else { t = fractalVal * 0.75 + patternVal * 0.25; }
        t = Math.max(0.03, Math.min(0.97, t));
        
        if (isGrail && anomalyClass) {
            if (anomalyClass === "Interference") {
                if (engineType === "Canonical") { const t2 = Math.sin((fractalVal + patternVal) * Math.PI * 12 / 8); t = (t + ((t2 + 1) / 2)) * 0.5; }
                else if (engineType === "Echo") { const echoMix = Math.cos((fractalVal * 0.6 + patternVal * 1.4) * Math.PI * 2.5); t = t * 0.65 + ((echoMix + 1) / 2) * 0.35; }
                else { const t2 = Math.sin((fractalVal - patternVal) * Math.PI * 20); t = Math.abs(t - ((t2 + 1) / 2)); }
                ...
                else if (engineType === "Echo") { const loop = Math.sin(rr * 9 + t * Math.PI * 4) * 0.5 + 0.5; t = t * 0.5 + loop * 0.5; }
                else t = t * 0.6 + Math.abs(ring) * 0.4;
            } else if (anomalyClass === "SpectralSplit") { t = Math.pow(t, engineType === "Rupture" ? 0.45 : 0.6); }
            t = Math.max(0.03, Math.min(0.97, t));
        ...
        if (variation < 0.02) { t += (Math.sin(rx * 12.3 + ry * 7.1) * 0.5 + 0.5) * 0.12; }
        if (variation < 0.01) {
            const fallback = Math.sin(rx * 8 + ry * 8) * 0.5 + 0.5;
            if (failureMode === "Recovering") { if (engineType === "Canonical") t = t * 0.65 + fallback * 0.35; else if (engineType === "Echo") t = t * 0.75 + fallback * 0.25; else t = t * 0.90 + fallback * 0.10; }
            else if (failureMode === "Residual") t = t * 0.88 + fallback * 0.12;
            else if (failureMode === "VoidBloom") { const bloom = Math.exp(-(rx * rx + ry * ry) * 1.8); t = t * 0.7 + bloom * 0.3; }
            else if (failureMode === "Fracture") { const crack = Math.abs(Math.sin(rx * 18 - ry * 11)); t = t * 0.6 + crack * 0.4; }
        }
        t = Math.max(0.03, Math.min(0.97, t));
        ...
        if (mode === "live" && liveIntensity !== undefined) {
            finalT = applyEngineLiveBehavior(finalT, engineType, liveIntensity, liveTime, pressure, traits, rx, ry, echoMemoryBuffer);
            if (liveTime !== undefined) {
                let warp; const warpStrength = 0.12 + pressure * 0.18;
                if (engineType === "Canonical") { warp = Math.sin(finalT * Math.PI * 2 + liveTime * 2) * (0.1 + pressure * 0.1); finalT = finalT * (0.88 - pressure * 0.06) + warp * warpStrength; }
                else if (engineType === "Echo") { const warp1 = Math.sin(finalT * Math.PI * 4 + liveTime * 3) * 0.08; const warp2 = Math.sin(finalT * Math.PI * 8 + liveTime * 1.2) * 0.05; warp = (warp1 + warp2) * (0.8 + pressure * 0.4); finalT = finalT * (0.82 - pressure * 0.08) + warp * warpStrength; }
                else { const warp1 = Math.abs(Math.sin(liveTime * 10 + finalT * 6)) * (0.2 + pressure * 0.2); const warp2 = Math.sin(finalT * Math.PI * 12 + liveTime * 8) * 0.1; warp = (warp1 + warp2) * 0.8; finalT = finalT * (0.75 - pressure * 0.12) + warp * warpStrength; }
        ...
        if (isGrail && anomalyClass === "SpectralSplit") {
            if (engineType === "Canonical") { r = Math.min(1, r * (1.18 + pressure * 0.1)); g = g * (0.78 - pressure * 0.08); b = Math.min(1, b * (1.08 + pressure * 0.08)); }
            else if (engineType === "Echo") { r = r * (0.82 - pressure * 0.04) + (Math.sin(time + finalT * 10) * 0.5 + 0.5) * (0.28 + pressure * 0.1); g = g * (0.7 - pressure * 0.05); b = b * (0.82 - pressure * 0.04) + (Math.cos(time + finalT * 10) * 0.5 + 0.5) * (0.28 + pressure * 0.1); }
            else { r = Math.min(1, r * (1.35 + pressure * 0.15)); g = g * (0.58 - pressure * 0.08); b = Math.min(1, b * (1.18 + pressure * 0.1)); }
        }
        
        // Apply micro-event at the very end
    ...
    function renderCanonical(tokenId, txHash, width = CONFIG.RENDER_WIDTH, height = CONFIG.RENDER_HEIGHT) {
        ...
        for (let y = 0; y < height; y++) { for (let x = 0; x < width; x++) { const pixel = computePixel({ x, y, width, height, traits, baseTraits, time: canonicalTime, intensity: canonicalIntensity, freqIndex, mode: "canonical" }); const idx = (y * width + x) * 4; pixels[idx] = pixel.r; pixels[idx+1] = pixel.g; pixels[idx+2] = pixel.b; pixels[idx+3] = 255; } }
        ...
        const metadata = { name: `Dart GenXL #${tokenId}`, description: `Deterministic generative art. Engine: ${traits.engineType}. Rarity: ${traits.rarityClass}.`, canonicalIntensity, canonicalTime, attributes: [
            { trait_type: "Rarity Class", value: traits.rarityClass }, { trait_type: "Engine Type", value: traits.engineType }, { trait_type: "Engine Variant", value: traits.engineVariant }, { trait_type: "Archetype", value: traits.archetype }, { trait_type: "Anchor Form", value: traits.anchorForm }, { trait_type: "Primary Driver", value: traits.primaryDriver }, { trait_type: "Structure Type", value: traits.structureType }, { trait_type: "Color Mood", value: traits.colorMood }, { trait_type: "Spatial Behavior", value: traits.spatialBehavior }, { trait_type: "Failure Mode", value: traits.failureMode }, { trait_type: "Anomaly Class", value: traits.anomalyClass || "None" }, { trait_type: "Micro Event", value: traits.microEvent }, { trait_type: "Canonical Intensity", value: canonicalIntensity.toFixed(3) }, { trait_type: "Event Score", value: eventScore.toString() }, { trait_type: "Stability Class", value: stabilityClass }, { trait_type: "Intensity Bias", value: intensityBias }, { trait_type: "Activation Type", value: activationType }
        ] };
        return { seed, traits, baseTraits, canonicalIntensity, canonicalTime, pixels, metadata, width, height, freqIndex, eventScore, stabilityClass, intensityBias, activationType };
    }
    
    // Helper functions for derived traits
    ...
    // ============================================================
    // LIVE SESSION (with session-scoped echo memory & intensity clamping)
    // ============================================================
    ...
    function createLiveSession(tokenId, txHash) {
        ...
        // 🔒 SESSION-SCOPED echo memory (no cross-token bleed)
        ...
        function renderFrame() {
            ...
            if (isFirstFrame) {
                // 🔒 FRAME 0: EXACT canonical render (no flash)
            ...
            const avgPixel = (pixels[0] + pixels[1] + pixels[2]) / (255 * 3);
            echoMemoryBufferSmooth = echoMemoryBufferSmooth * 0.85 + avgPixel * 0.15;
        ...
        function startAnimation(canvas) {
            ...
            fetchIntensity();
            const intervalId = setInterval(fetchIntensity, 15000);
        ...
        function setLiveIntensity(val) { 
            let newVal = Math.max(0.05, Math.min(0.95, val));
            if (Math.abs(newVal - targetLiveIntensity) > 0.3) {
                newVal = targetLiveIntensity + Math.sign(newVal - targetLiveIntensity) * 0.3;
            }
            targetLiveIntensity = newVal; 
        }
        
        return { startAnimation, stop, getCanonical: () => canonical, getLiveIntensity, setLiveIntensity };
    }
    ...
    // ✅ NO auto-init - viewer controls everything
    
    console.log("Dart GenXL Engine v1.8 - Added micro-variants, seeded modifiers, engine variants, new spatial behaviors");
})();
