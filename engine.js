...
// PREVIEW SEED SUPPORT - Teaser images different from mint
// ============================================================
...
(function() {
    ...
    // ============================================================
    // PREVIEW SEED (tokenId only - NO txHash)
    // ============================================================
    ...
    // ============================================================
    // PREVIEW RENDER (tokenId only - different from mint)
    // ============================================================
    
    function renderPreview(tokenId, width = CONFIG.RENDER_WIDTH, height = CONFIG.RENDER_HEIGHT) {
        const previewSeed = getPreviewSeed(tokenId);
        
        // Generate traits using preview seed
        const traits = generateTraits(previewSeed, tokenId);
        const baseTraits = generateBaseTraits(previewSeed, tokenId);
        
        // Preview intensity (different range from canonical)
        const previewIntensityRng = splitmix64(splitSeed(previewSeed, 200));
        const previewIntensity = 0.3 + previewIntensityRng() * 0.5;
        
        const previewTime = (parseInt(tokenId, 10) * 0.0123456789) % 1.0;
        
        const freqRNG = makeSeededRand(splitSeed(previewSeed, 300));
        const freqIndex = Math.floor(freqRNG() * 4);
        ...
        for (let y = 0; y < height; y++) { 
            for (let x = 0; x < width; x++) { 
                const pixel = computePixel({ 
                    x, y, width, height, traits, baseTraits, 
                    time: previewTime, intensity: previewIntensity, 
                    freqIndex, mode: "canonical" 
                }); 
                const idx = (y * width + x) * 4; 
                pixels[idx] = pixel.r; 
        ...
        return { 
            seed: previewSeed, traits, baseTraits, 
            canonicalIntensity: previewIntensity, 
            pixels, width, height, freqIndex, eventScore,
    ...
    // ============================================================
    // LIVE SESSION (simplified for preview)
    // ============================================================
    
    function createLiveSession(tokenId, txHash) {
        const canonical = renderCanonical(tokenId, txHash);
        // Simplified for now - returns canonical
        return {
            startAnimation: (canvas) => drawToCanvas(canvas, canonical),
            stop: () => {},
            getCanonical: () => canonical,
            getLiveIntensity: () => canonical.canonicalIntensity,
            getDisplayIntensity: () => canonical.canonicalIntensity
        };
    }
    ...
    console.log("Dart GenXL Engine v1.9.0 - Preview seed support added");
})();
