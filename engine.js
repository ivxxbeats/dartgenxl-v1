...
// NO FLASH: Canonical first, pressure ramps with fade
// ============================================================

(function() {
    "use strict";
    
    const CONFIG = { MODE: null, RENDER_WIDTH: 640, RENDER_HEIGHT: 640, SEED_SALT: "dart_hlgen_v1" };
    ...
    function splitmix64(seed) {
        let s = seed >>> 0;
        return function() { s = s + 0x9e3779b9 | 0; let t = s ^ s >>> 16; t = Math.imul(t, 0x21f0aaad); t = t ^ t >>> 15; t = Math.imul(t, 0x735a2d97); return ((t = t ^ t >>> 15) >>> 0) / 4294967296; };
    }
    ...
    // ============================================================
    // TRAIT GENERATION (abbreviated for brevity - same as before)
    // ============================================================
    ...
    function rollSpatialBehavior(engineType, rng) { const allowed = { "Canonical": ["Radial","Spiral","Kaleido"], "Echo": ["FlowField","Vortex","Asymmetrical"], "Rupture": ["Asymmetrical","Vortex","Radial"] }; const options = allowed[engineType] || allowed.Canonical; return options[Math.floor(rng() * options.length)]; }
    ...
    function generateTraits(seed, tokenId) {
        const streams = {}; for (let i = 1; i <= 10; i++) streams[i] = makeSeededRand(splitSeed(seed, i));
        const tokenNum = parseInt(tokenId, 10) || 0;
        const steps = (tokenNum * 997) % 1000;
        for (let i = 0; i < steps; i++) for (let s = 1; s <= 10; s++) if (streams[s]) streams[s]();
        const rng = streams[1];
    ...
    function generateBaseTraits(seed, tokenId) {
        const streams = {}; for (let i = 1; i <= 7; i++) streams[i] = makeSeededRand(splitSeed(seed, i + 100));
        const tokenNum = parseInt(tokenId, 10) || 0;
        const steps = (tokenNum * 997) % 1000;
        for (let i = 0; i < steps; i++) for (let s = 1; s <= 7; s++) if (streams[s]) streams[s]();
        const rng = streams[1];
        ...
        return { zoom: (0.5 + rng() * 0.8) * zoomVariety, offsetX: (rng() - 0.5) * 1.0 + offsetVarietyX, offsetY: (rng() - 0.5) * 1.0 + offsetVarietyY, baseMaxIter: 60 + Math.floor(rng() * 140) + iterVariety, iterMult, layers };
    }
    ...
    function getPatternValue(x, y, time, engineName, pressure, primaryDriver, rx, ry, liveTime) {
        ...
        if (primaryDriver === "Pattern") val += Math.sin(rx * 10 + ry * 10 + liveTime) * 0.15 * pressure;
    ...
    function computePixel(options) {
        ...
        return { r: Math.floor(Math.min(255, Math.max(0, r * 255))), g: Math.floor(Math.min(255, Math.max(0, g * 255))), b: Math.floor(Math.min(255, Math.max(0, b * 255))) };
    }
    ...
    function createLiveSession(tokenId, txHash) {
        ...
        function renderFrame(fadeFactor = 1) {
            ...
            const pixels = new Uint8ClampedArray(width * height * 4);
            for (let y = 0; y < height; y++) { for (let x = 0; x < width; x++) { const pixel = computePixel({ x, y, width, height, traits, baseTraits, time: canonicalTime, intensity: canonicalIntensity, freqIndex, mode: "live", liveIntensity: currentLiveIntensity, liveTime: currentLiveTime, fadeFactor }); const idx = (y * width + x) * 4; pixels[idx] = pixel.r; pixels[idx+1] = pixel.g; pixels[idx+2] = pixel.b; pixels[idx+3] = 255; } }
            ...
            ctx.fillStyle = "rgba(0,0,0,0.6)"; ctx.fillRect(10, height - 30, 100, 5);
            let badgeText = "";
            if (pressure > 0.82) badgeText = "🔥 ASCENDED";
            else if (pressure > 0.60) badgeText = "✨ AWAKENED";
            else if (pressure > 0.35) badgeText = "⚡ STIRRING";
            if (badgeText) { ctx.fillStyle = `hsl(${pressure * 120}, 100%, 55%)`; ctx.fillRect(10, height - 30, currentLiveIntensity * 100, 5); ctx.font = "bold 10px monospace"; ctx.fillStyle = `hsl(${pressure * 120}, 100%, 60%)`; ctx.fillText(badgeText, width - 80, height - 20); }
            else { ctx.fillStyle = `hsl(${currentLiveIntensity * 120}, 100%, 55%)`; ctx.fillRect(10, height - 30, currentLiveIntensity * 100, 5); }
        }
        
        function fetchIntensity() { fetch('https://raw.githubusercontent.com/ivxxbeats/farcaster-intensity/main/intensity.json?t=' + Date.now()).then(r => r.json()).then(data => { if (data && typeof data.intensity === 'number') targetLiveIntensity = Math.max(0.05, Math.min(0.95, data.intensity)); }).catch(() => console.warn("Intensity fetch failed")); }
        ...
        function startAnimation(canvas) {
            ...
            fetchIntensity(); setInterval(fetchIntensity, 15000);
            animationId = requestAnimationFrame(animate);
        }
        
        function stop() { if (animationId) { cancelAnimationFrame(animationId); animationId = null; } }
        function updateInfoPanel() { const infoDiv = document.getElementById('info'); if (infoDiv) infoDiv.innerHTML = `${canonical.traits.engineType} · ${canonical.traits.rarityClass} · ${canonical.traits.archetype} · ${canonical.traits.anchorForm} · ${canonical.traits.primaryDriver} · ${canonical.traits.failureMode} · ${canonical.traits.anomalyClass || 'None'} · ES: ${canonical.eventScore || 0}`; }
    ...
    function autoInit() {
        ...
        if (freeze) {
            ...
            if (infoDiv) { const t = result.traits; infoDiv.innerHTML = `${t.engineType} · ${t.rarityClass} · ${t.archetype} · ${t.anchorForm} · ${t.primaryDriver} · ${t.failureMode} · ${t.anomalyClass || 'None'} · ES:${result.eventScore || 0}`; }
        } else {
    ...
    console.log("Dart HLGEN v1.6 - NO FLASH: Live mode draws canonical first, pressure ramps with fade");
})();
